...
ClapTrap::ClapTrap(const ClapTrap &C) {
	std::cout << "ClapTrap copy constructor called for " << C._name << std::endl;
	*this = C;
}

ClapTrap& ClapTrap::operator=(ClapTrap const& C) {
	std::cout << "ClapTrap copy assignement operator called for " << C._name << std::endl;
    this->_name = C._name;
...
void ClapTrap::attack(const std::string& target) {
	if (!this->isDead()) {
		this->_energy_pts--;
		std::cout << this->_name << " Clap attacks " << target << ", causing " << this->_attack_dmg << " points of damage" << std::endl;
	}
}
